...
<!-- beta:voice_layer -->
**With the voice layer on** the enum in full is note|done|request|escalation|ingest|**voice** — `voice` is the owner speaking through their voice buddy. It renders as `[MSG from buddy · voice]`; its sender is listening, not watching your terminal, so answer it with a message back rather than a line in your own pane.
