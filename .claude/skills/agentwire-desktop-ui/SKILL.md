...
**Z-index landscape:** WinBox windows (inline, grows from 10) < phantom topology overlay (1000) < collage overlay (1400) < toasts (1500) < modals (2000) < command palette (3000) < sidebar (9001) < tile drag overlay (99999).
...
- **Phantom overlay** (`topology-overlay.js`, #764) — hosts `TopologyView` (`mode: 'overlay'`) as a transient, non-interactive pop-over: the live `session_created` event pops the new child's family over the terminals, lingers, then fades. `pointer-events: none` cascades from the root through every card (inherited CSS property) except the small dismiss button. Settings toggle persists to `localStorage['aw-topology-overlay-visible']` (Config sidebar checkbox); per-session dismiss hides the current pop without touching the toggle. z-index 1000 — above windows, below the collage (1400). Supersedes the old connector overlay (`topology-wires.js`, #746, deleted by #764) — the wire overlay only read sensibly on a wide tiled desktop the owner never runs.
- **Grouped + tinted collage** (`collage.js`, #748) — see "Window Collage" above.
- **Live appearance** (`desktop.js` `handleSessionCreated` + `agentwire/core.py` `notify_portal_session_created`, #747) — `agentwire new`/`worktree` posts a `session_created` event as soon as the session exists, so the desktop merges it into the live list (and can trigger born-from-parent placement + the phantom overlay) without waiting for the next `sessions_update` poll.
