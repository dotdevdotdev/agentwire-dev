...
```bash
agentwire limits tick          # one watchdog pass (what the scheduler runs)
...
agentwire limits install       # install + start the watchdog (60s tick)
agentwire limits uninstall
```

`limits install` picks the platform backend: on macOS a launchd LaunchAgent
(`dev.agentwire.usage-limit-watchdog`), on Linux a systemd `--user`
timer+service pair (`agentwire-usage-limit-watchdog.timer` in
...
Logs: `~/Library/Logs/agentwire-usage-limit-watchdog.log` on macOS,
`journalctl --user -u agentwire-usage-limit-watchdog` on Linux.
...
- Watchdog not running? macOS: `launchctl list | grep usage-limit` and
  `tail -f ~/Library/Logs/agentwire-usage-limit-watchdog.log`. Linux:
  `systemctl --user status agentwire-usage-limit-watchdog.timer` and
  `journalctl --user -u agentwire-usage-limit-watchdog -f`.
