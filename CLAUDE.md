...
When the owner says "worktree session", they mean the standalone session (`agentwire worktree`), **never** `spawn` panes. Worker panes are for small subtasks watched by the orchestrator; worktree sessions are for parallel autonomous work. Posture defaults by TOPOLOGY, not role: standalone sessions (`worktree`/`new`, whichever role) default to the bypass posture (full access — isolated or it's your own driver seat); `spawn` (a pane, watched in real time) defaults to restricted — no `--posture` needed for the common case.
