...
**Worktree lifecycle (MCP):** `worktree_create(name, project_dir, roles, base, prompt, created_by, kind)` spawns a worktree session (new branch + checkout + tmux session, optionally seeded with `prompt`); the new session is only parented to you when `project_dir` is your own project (#715) — pass `created_by` to force a specific parent for a closely related project instead. `kind` is the ROLE axis (#716, independent of topology) — `"worker"` (default: safety-railed isolation/verify/draft-PR/notify) or `"orchestrator"` (durable, replaceable-persona project window; roots by default unless `created_by` is given). `worktree_status` / `worktree_list(project_dir, check_dangling)` (read-only git status; `check_dangling=True` also flags LIVE **worker**-kind sessions with an OPEN PR and no live parent — orchestrator-kind entries are excluded since a self-rooted orchestrator with an open PR is its normal healthy lifecycle; distinct from the "orphan" state, a dead session with a disk remnant). `worktree_remove(name, project_dir, keep_branch, force_delete_branch)` is an ATOMIC teardown — kill session + force-remove the git worktree + delete the branch (local + remote, once confirmed merged via `gh pr view` + a headRefOid tip-SHA check, or a git ancestor fallback) + unregister; it fails LOUDLY (never silently "unregisters" an orphan) if the directory can't actually be cleared, and never needs `main` checked out anywhere. `worktree_prune(project_dir, gc_merged)` drops stale registry entries (dir already gone); `gc_merged=True` additionally tears down any still-present entry whose branch is confirmed merged. Worker *panes* via `pane_spawn` still share the orchestrator's working dir — use those for quick subtasks, worktree sessions for isolated parallel work.
