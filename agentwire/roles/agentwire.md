...
**Polite vs forceful.** Prefer `msg_send` for routine peer updates that must not interrupt — "PR drafted", "picking up the footer". It drops the message into a file inbox and the watchdog injects it only when the recipient's input box is empty (≤60s), so it **never clobbers a human's half-typed draft**. Reach for `session_send` **only** when you must drive a session right now — it pastes + Enter immediately, overwriting any uncommitted draft. `kind` ∈ note|done|request|escalation|**ingest**|**voice** (`ingest` is passive — never auto-delivered, pull-only; `voice` is the owner speaking through their voice buddy); `to="@all"` broadcasts to live agent sessions except you.
