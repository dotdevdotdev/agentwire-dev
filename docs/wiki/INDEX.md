...
- **[Voice Layer](voice-layer.md)** — ⚗️ **EXPERIMENTAL spike, branch-only, not for merge.** A realtime voice buddy (`gpt-realtime-2.1` over WebRTC) the owner talks to *about* the fleet — read-only fleet awareness, a session identity with no tmux session, and one inbox delivery adapter. Explicitly **not a harness**: never writes code, never owns a worktree, never appears in the topology
