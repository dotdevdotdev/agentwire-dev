...
- **Kokoro-82M is the default voice** — `tts.backend: default` now speaks via an in-process Kokoro engine (CPU ONNX, torch-free, 32 presets / 8 languages), bundled with the base install (`kokoro-onnx`, py<3.14). The portal downloads the model (~200 MB, one-time, atomic) in the background on first start with a progress toast; browser `speechSynthesis` covers speech until ready and remains the last-resort fallback (model unavailable / py3.14+). No-browser playback upgraded from `say`/`espeak` to Kokoro on local speakers. New `agentwire tts warm` pre-downloads for headless setups; `agentwire tts status` reports default-tier model state; `/api/voice-status` exposes `tts.kokoro {state, percent}`; `/api/voices` + `voiceclone list` surface the presets. The `custom` shim tier is unchanged (chatterbox venvs now need Python 3.13+ — numpy 2 gate). `audio_done` timing now parses real WAV duration instead of a 2x-off stereo estimate. Issue [#269](https://github.com/dotdevdotdev/agentwire-dev/issues/269)
